...
- (UIImage *)createTextBitmapWithText:(NSString *)text
  ...
  [attributedText drawAtPoint:CGPointMake(textX, textY)];
  ...
  if (label && label.length > 0) {
    ...
    NSAttributedString *attributedLabel = [[NSAttributedString alloc] initWithString:label attributes:labelAttributes];
    [attributedLabel drawAtPoint:CGPointMake(labelX, labelY)];
  }
...
- (void)addTextMarker:(NSDictionary *)textMarkerOptions result:(OnDictionaryResult)completionBlock {
  ...
  NSString *textColorStr = [textMarkerOptions objectForKey:@"textColor"];
  UIColor *textColor = textColorStr ? [UIColor colorWithHexString:textColorStr] : [UIColor blackColor];
